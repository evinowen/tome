...
const create = async (path: string) => {
  ...
  const config_item = await files.create({ path, name: 'config.json' })
  const config_content = `${JSON.stringify(TemplateBaseConfiguration, undefined, 2)}\n`
  await files.save({ item: config_item, content: config_content })
  await files.select({ item: config_item })

  const index_item = await files.create({ path, name: 'index.md' })
  await files.save({ item: index_item, content: TemplateBaseIndex })
...
const execute = async (data: FeatureExecuteInput) => {
  ...
  if (success) {
    if (result) {
      ...
      await files.select({ path: result })
    }

    await log.info(`Template ${name} complete`)
  } else {
    await log.error(`Template ${name} failed: ${result}`)
  }
}

export const fetch_templates_store = defineStore('templates', {
  state: StateDefaults,
  actions: {
    load: async function ({ path }) {
      const absolute = await api.path.join(path, '.tome', 'templates')
    ...
    prepare: async function () {
      ...
      for (const target of targets) {
        if (!parent) {
          throw new Error('Cannot ensure templates feature file structure.')
        }
