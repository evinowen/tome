import { assemble } from '?/helpers'
import Vuetify from 'vuetify'
import store from '@/store'
import LibraryButton from '@/components/LibraryButton.vue'

jest.mock('@/store', () => ({
  state: {
    library: {
      history: []
    },
    repository: {
      path: './tome_path'
    }
  },
  dispatch: jest.fn()
}))

describe('components/LibraryButton', () => {
  let vuetify
  let value
  let stored

  const factory = assemble(LibraryButton, { value, stored })
    .context(() => ({ vuetify }))

  beforeEach(() => {
    vuetify = new Vuetify()
  })

  afterEach(() => {
    jest.clearAllMocks()
  })

  it('should dispatch library/select when select is called', async () => {
    const wrapper = factory.wrap()
    const local = wrapper.vm as LibraryButton
    await local.select()

    const mocked_store = jest.mocked(store)
    const [action] = mocked_store.dispatch.mock.calls.find(([action]) => (action as unknown as string) === 'library/select')

    expect(action).toBeDefined()
  })

  it('should dispatch library/open with path when open is called with a path', async () => {
    const path = '/project'

    const wrapper = factory.wrap()
    const local = wrapper.vm as LibraryButton
    await local.open(path)

    const mocked_store = jest.mocked(store)
    const [action, data] = mocked_store.dispatch.mock.calls.find(([action]) => (action as unknown as string) === 'library/open')

    expect(action).toBeDefined()
    expect(data).toBe(path)
  })

  it('should dispatch library/close when close is called', async () => {
    const wrapper = factory.wrap()
    const local = wrapper.vm as LibraryButton
    await local.close()

    const mocked_store = jest.mocked(store)
    const [action] = mocked_store.dispatch.mock.calls.find(([action]) => (action as unknown as string) === 'library/close')

    expect(action).toBeDefined()
  })
})
